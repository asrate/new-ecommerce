import React from "react";
import classes from "./product.module.css";
import Rating from "@mui/material/Rating";
function MuradeProductCard({singleProduct }) {
//   const { title, image, price, rating, id } = singleProduct;
 // <div key={product.id}>
        //   <h2>{product.name}</h2>
        //   <p>Stock Amount: {product.stockAmount}</p>
        //   <img src={product.image} alt="" />
        //   {/* Add other product details as needed */}
        // </div>
  console.log(singleProduct);
//   const truncatedTitle = ashewaProduct.name.length > 20 ? ashewaProduct.name.substring(0, 20) + "..." : ashewaProduct.name;
  return (
    
    <div className={classes.productCard_container}>
    <a href={`https://muradezema.com/product/${singleProduct.slug}`} target="_blank" rel="noopener noreferrer">

        <img src={singleProduct.images[0].src} />
    
      <div>
        <h3>{singleProduct.name}</h3>
      </div>
     
      <div>
        <p>Price {singleProduct.price} Br</p>
      </div>
      <div>
        <button className={classes.button}>SEE IT</button>
      </div>
      {/* Ashewa */}
      </a>
    </div>
    
    
  );
}

export default MuradeProductCard;
